...
Three entry points work on a case without writing any Python. They all take the same
...
and covering a smaller subset. :doc:`cli_rdates` does not plot: it lists a case's report steps
with their dates and the time since the simulation started - handy for picking the ``--rstep``
to pass to the other two.
...
``--calculator`` - used throughout the examples on the program pages.
